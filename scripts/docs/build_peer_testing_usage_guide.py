...
def build_doc(output_path: Path) -> Path:
    ...
    add_table(
        ...
        ["项目", "地址 / 账号", "当前状态与备注"],
        [
            ["Web 管理端", "http://10.10.0.13/", "推荐主入口；2026-05-25 实测 HTTP 200。"],
    ...
    add_paragraph(doc, "2.3 当前默认数据状态（2026-05-25 核实）", style="Heading 2")
    add_table(
        doc,
        ["模块 / 数据", "当前状态", "互测建议"],
        [
            ...
            ["知识条目", "0 条", "若要测知识检索，请先在 Web 后台创建并发布一条知识条目。"],
        ],
        [1.45, 1.45, 3.6],
    )
    ...
    add_bullets(
        ...
        [
            ...
            "知识库和党团流程实例目前没有预置完整演示数据，因此部分模块需要测试者主动创建一条测试数据。",
    ...
    add_table(
        ...
        [
            ...
            ["知识库", "先新增来源，再新增一条标题带“[互测]”的知识条目并发布。", "因为当前默认没有预置知识正文数据。"],
    ...
    add_bullets(
        doc,
        [
            "知识库模块当前没有预置已发布知识条目，所以默认状态下搜索可能返回 0 条结果；这不是页面崩溃，但会影响“开箱即搜即答”的体验。",
