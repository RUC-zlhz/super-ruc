...
4. **Given** 学生发起证明申请，**When** 系统加载系统内置证明版式和本人基础信息，**Then** 系统可生成 PDF 预览供学生与审批人查看；当前阶段不承诺完整 Word 模板字段映射引擎。
...
- **FR-006**: 系统 MUST 支持学生在线提交请假、盖章、证明、报名与材料提交等常见事务申请，并在证明场景支持基于系统内置版式生成 PDF 预览；完整模板字段映射引擎不属于当前阶段承诺。
...
- PDF 成绩单和证明附件分开处理：  
  - **成绩单 PDF**：先解析文本层，再映射到成绩暂存结构；上传后统一进入教师核验批次，教师确认提交前不得更新正式成绩或学业提示结果；
...
- **knowledge_source**: 存储来源、官方标识、链接和版本元数据。关键字段：`source_id bigserial`、
  `source_name varchar(255)`、`source_url varchar(1024)`、`is_official boolean`、
  `version_label varchar(64)`、`effective_date date`、`expired_date date`；官方来源必须填写 `source_url`，创建和修改需写入审计日志。 [FR-002][FR-013]
...
- **SC-007**: 90% 的标准证明申请可基于系统内置版式生成 PDF 预览，并在审批链中保持同一份预览引用；完整 Word 模板自动填充不作为当前验收口径。
