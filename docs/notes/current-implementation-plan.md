...
- 细化文件：`docs/notes/refinements/2026-05-28-s66-admin-initial-password-copy-normalization.md`
- 当前状态：`[x]` 已完成问题定位、文案修复与 Web 验证。
- [x] `S66.1` 复核 `must_change_password` 触发路径，确认问题在 Web 提示文案硬编码，不在后端鉴权逻辑。
- [x] `S66.2` 修改登录页、个人信息页和改密弹窗提醒文案，去掉写死的 `admin123`。
- [x] `S66.3` 运行 Web 构建验证并回写结果。

当前结论：

- `must_change_password` 仍保留“首次登录需改密”的安全语义。
- 新管理员首次登录不再看到误导性的 `admin123` 固定提示，默认超管 `admin/admin123` 的种子与运维口径保持不变。
- 本轮已通过 `web vue-tsc` 和 `pnpm -C web build` 验证。
...
- 细化文件：`docs/notes/refinements/2026-06-01-s68-peer-received-bug-fixes.md`
- 当前状态：`[x]` 已完成 6 条收到反馈的代码复核、有效修复、回归测试补充和静态/构建/定向 DB 集成验证。
- [x] `S68.1` 读取互测平台第 12 组 `/bugs/received`，确认本轮范围为 `229 / 218 / 217 / 191 / 160 / 162`。
- [x] `S68.2` 修复 Miniapp 通知时间展示，避免直接截取 UTC ISO 字符串导致小程序端比 Web 端少 8 小时。
- [x] `S68.3` 增强 Miniapp 模板下载，优先走认证文件流接口，失败后回退预签名链接，并区分下载失败与设备无法预览。
- [x] `S68.4` 为知识条目“带实际来源 URL 保存草稿”补后端回归；当前 HEAD 不复现服务器内部错误，回归已通过。
- [x] `S68.5` 改善 Web 学生画像快照下载错误反馈，避免下载失败时无反应。
- [x] `S68.6` 修复培养方案空课程明细展开区，0 门课程时显示稳定空态，不渲染会拉长页面的空表格。
- [x] `S68.7` 复核 `ID 162` 已由 `S67` 覆盖，保留既有前后端日期顺序校验与回归用例。

当前结论：

- `ID 229 / 218 / 191 / 160` 已完成代码修复。
- `ID 217` 在当前 HEAD 未复现，新增回归用例锁定“带 URL 来源的知识条目保存草稿”，定向 DB 集成已通过。
- 静态与构建验证通过：后端 ruff、后端 py_compile、Miniapp vue-tsc、Web build、Miniapp mp-weixin build。
- 定向 DB 集成回归通过：S68 四条用例 `4 passed in 76.87s`。
- `ID 162` 已由 `S67` 代码修复覆盖，本轮纳入 S68 状态说明。

### S69 仓库本地缓存忽略与 GitHub 同步

- 细化文件：`docs/notes/refinements/2026-06-03-s69-repo-hygiene-github-upload.md`
- 当前状态：`[x]` 已完成仓库本地缓存忽略规则整理、Git 状态复核、提交与 GitHub 推送。
...
- [x] `S69.3` 在 `.gitignore` 补充根目录本地 Python/uv/ruff 缓存忽略规则，避免本地缓存权限或临时产物污染 Git 状态。
...
### S73 小程序时间格式化 Intl 运行时兼容修复

- 细化文件：`docs/notes/refinements/2026-06-08-s73-miniapp-intl-datetime-runtime-fix.md`
- 当前状态：`[x]` 已完成。
- [x] `S73.1` 定位小程序运行时报错根因：`formatShanghaiDateTime` 直接调用 `Intl.DateTimeFormat`，微信小程序部分运行环境缺少全局 `Intl`。
- [x] `S73.2` 修改 `miniapp/src/utils/datetime.ts`，用时间戳加 `UTC+8` 后的 UTC getter 手动拼接上海时间字符串。
- [x] `S73.3` 运行 Miniapp 类型检查、`mp-weixin` 构建与产物 `Intl` 残留扫描。
- [x] `S73.4` 提交并推送 GitHub。
...
- [ ] `S74.5` 最终整合验证后，在生产环境做附件下载验收。

当前结论：

- 老师后续入口为：管理后台 `审批工作台 -> 审批详情 -> 附件列表 -> 下载`。
- 代码已由 S76 主线修复复用并通过本地定向 DB 集成测试；最终合并 S75 后还需重新跑整合验证。

### S76 第 12 组 Pending 反馈修复闭环

- 细化文件：`docs/notes/refinements/2026-06-26-s76-peer-pending-bug-closure.md`
- 当前状态：`[x]` 已完成。
- [x] `S76.1` 从 `origin/main` 新建隔离 worktree `D:\Codes\super-ruc-wt\s76-peer-pending-bugs`，不混入根目录未提交的 S74/S75 半成品。
- [x] `S76.2` 为申请附件补认证态下载接口与审批详情下载按钮，覆盖反馈 `#346 / #328`。
- [x] `S76.3` 将用户管理与理论自测题库查询按钮改为显式点击触发，覆盖反馈 `#344 / #327`。
- [x] `S76.4` 为理论自测题库组合筛选补后端回归，锁定 `topic / qtype / q / is_active` 参数。
- [x] `S76.5` 为通知生效日期增加前端提示、Pydantic 入参校验，以及发布/发送历史非法数据兜底，覆盖反馈 `#326`。
- [x] `S76.6` 本地 Docker/DB 定向验证、后端静态验证与 Web 构建。
- [x] `S76.7` 提交、推送、监督 GitHub Actions 部署并通过 `n150` 做生产只读 smoke。

当前结论：

- `d0b48c9c59c076b8058676b8c464c2949736c567` 已推送到 `origin/main`，`Intranet Production Deploy` run `28224581224` 成功；经 `ssh n150` 只读检查生产 `/healthz`、首页与未登录后台 API，结果符合预期。
...
- 细化文件：`docs/notes/refinements/2026-06-29-s80-template-superadmin-honor-qa-fixes.md`
- 当前状态：`[x]` 已完成。
- [x] `S80.1` 学生端模板列表改为展示所有 `ACTIVE` 模板，不再要求模板必须关联已发布知识条目。
- [x] `S80.2` 学生端模板下载同步移除旧发布关联门禁，认证用户可下载可用模板。
- [x] `S80.3` 为模板文件补 `tags` 字段、迁移、上传解析和标签检索。
- [x] `S80.4` 管理端模板上传/列表与小程序常用模板列表补标签展示，并明确可用模板直接进入学生端。
- [x] `S80.5` `SUPER_ADMIN` 作为审批兜底角色，可审批未显式配置超管的申请类型。
- [x] `S80.6` 补定向回归并完成后端、Web、小程序验证。

当前结论：

- 本轮修复已通过后端静态检查、编译、两条定向 DB 集成回归、Web 类型检查与构建、小程序类型检查与 `mp-weixin` 构建；荣誉展示落点经代码核查为小程序 `pages/honor/index` 的“荣誉公示”和 Web 荣誉管理页。

### S81 审批与导出可读身份展示修复

- 细化文件：`docs/notes/refinements/2026-06-29-s81-human-readable-identities.md`
- 当前状态：`[-]` 进行中。
- [x] `S81.1` 申请列表和详情响应补申请人姓名、学号与账号名。
- [x] `S81.2` 审批详情响应补审批人姓名/工号与审批记录操作人身份。
- [x] `S81.3` Web 审批详情、审批工作台与 Miniapp 申请详情改为展示可读身份。
- [x] `S81.4` 成绩单导出改用 `student_no/full_name`，不再导出内部 `student_id` 作为识别列。
- [-] `S81.5` 定向回归与双端验证进行中。
...
| 2026-06-26 | S78 用户使用说明书与软件测试报告最终提交版出件 | `docs/notes/refinements/2026-06-26-s78-final-user-manual-and-test-report.md` | `S78.1 ~ S78.8` | `[x]` | 基于当前 HEAD `ca3b8de` 与 2026-06-26 生产在线抽检（经 n150），将用户手册与测试报告升级为 V2.0 最终提交版；复核 2026-05-26 基线 14 项 Logic 缺陷均已修复；以有效 `SUPER_ADMIN` 账号在生产完成认证态读写验证（11 类只读 code=0、知识/通知两条写入闭环实测并清理、印证 S75 缓存事件失效）；学生端↔教师审批闭环**用真实微信在生产端到端跑通**（微信开发者工具真实 wx.login→生产签发学生会话→请假申请→提交→辅导员审批→学生回看 APPROVED），证明 PDF/附件下载等细节由本地 `test_request_flow` 20 passed 复核。出件 `用户使用说明书…v2.0`（14 页）与 `软件测试报告…v2.0`（13 页），文本与版式 QC 通过 |
