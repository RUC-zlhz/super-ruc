# 2026-05-26 S51 党团平台文件 2 知识导入与学生端检索闭环
...
- [x] `S51.1` 从 4 份 PDF 中抽取可直接回答的常见问题、关键词与标准答复，按文件来源整理为 FAQ 型知识条目。
- [x] `S51.2` 新增显式导入脚本 `backend/scripts/import_party_platform_file2_knowledge.py`，将上述内容 upsert 为知识来源和已发布知识条目。
- [x] `S51.3` 增强学生侧搜索：`/knowledge/search` 除标题、摘要、正文外，额外支持标签和来源名称命中。
- [x] `S51.4` 增强学生侧智能匹配：`/knowledge/ai-match` 返回摘要、命中原因和来源文件；当整句 SQL 搜索未命中时，回退到已发布条目集合做关键词重排。
- [x] `S51.5` 增补知识库回归样例，覆盖“标签检索命中”和“自然问法命中摘要/原因”两条关键链路。
- [x] `S51.6` 在本地开发库完成一次真实导入发布，并用运行态接口复测典型查询。
