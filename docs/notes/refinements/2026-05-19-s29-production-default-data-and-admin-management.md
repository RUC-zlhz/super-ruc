...
- [x] `S29.5` 补 Web 管理入口：学生画像页新增“编辑学籍信息”，覆盖姓名、性别、年级、专业、班级、政治面貌、入学年份与预计毕业年份等主档字段；用户管理页新增“新增单个账号”。
...
- 追加字段扩展后，服务器已重建并启动 `backend` / `web`；`bash deploy/intranet-prod/scripts/smoke.sh` 通过，未登录探测 `PATCH /api/v1/admin/students/1/academic-info` 返回 `401` 而非 `404`。
...
- 学生主档维护入口为 `用户管理 -> 学生管理 -> 画像 -> 编辑学籍信息`，可维护姓名、性别、年级、专业、班级、政治面貌、入学年份与预计毕业年份；学籍生命周期状态仍在 `用户管理 -> 学生管理 -> 学籍` 中维护。
