...
**Version:** 1.2 | **Created:** 2026-04-13 | **Last Updated:** 2026-05-12
...
| S12 需求缺口闭环与默认数据导入 | `[x]` 已完成 | 9 个 S12 gap 已映射到既有 FR 的增量验收条目；后端 S12 定向回归 `5 passed`、原 report 合约回归 `4 passed`、Web 构建、小程序 `mp-weixin` 出包和 SRS v1.7 三组 DOCX/PDF 文档 QC 均已完成 |
| S13 需求文档与实现一致性修复 | `[x]` 已完成 | 已修正文档状态漂移、FR 验收项语义和需求边界表述；知识来源结构化官方标识、同分优先排序、管理端维护入口和双端消费已落地，并通过后端静态校验、隔离 Kingbase 定向集成回归 `5 passed`、Web 构建、小程序出包和文档 grep 检查 |

### S12 Incremental Traceability Addendum

| S12 Gap | Related FRs | Incremental Acceptance / Assumption |
|---------|-------------|--------------------------------------|
| 默认数据导入 | FR-009, FR-015 | 仅消费仓库登记的默认学生/培养方案源；不从学号或预计毕业年份推断专业、年级、班级；`2024-default` 仅作初始演示版本 |
| 成绩单 PDF 核验 | FR-009, FR-014 | 学生上传后只生成待人工核验批次；教师核验提交后才写正式成绩记录 |
| 学业缺口与课程推荐增强 | FR-014, FR-015 | 课程类型建议只基于已核验成绩、培养方案规则与可用开课数据；缺少容量、课表、先修或偏好时提示“数据未配置” |
| 模板下载与知识官方链接 | FR-002, FR-003 | 同等相关度下按结构化官方来源标识优先展示，并在存在 `source_url` 时回链；已发布模板可供学生侧下载，未发布或已停用模板不得开放 |
| 统一进度中心 | FR-004, FR-006, FR-007, FR-008 | 并列展示本人党团流程与相关事务申请进度，并保留详情入口；事务申请进度映射提交、审核、撤回、驳回重提和转线下等既有流转状态 |
| 受控通知抓取 | FR-010, FR-011 | 公众号通知默认手工录入；自动抓取仅限公开 URL/RSS 来源和管理员手工触发，不绕过登录、验证码或反爬限制；抓取结果先形成草稿通知和抓取历史，不自动发布或群发 |
| 短信投递治理 | FR-011 | SMS 仅以 mock/local provider 作为一期兜底；保留 retry、attempt 与 receipt 状态，不承诺真实运营商接入 |
| Web 管理端接入 | FR-007, FR-009, FR-010, FR-011, FR-014 | 系统内置证明版式 PDF 预览/下载、默认导入、成绩单核验、模板下载、通知来源/抓取历史与 SMS 重试入口均映射到既有 FR 的增量验收 |
| Miniapp 学生端接入 | FR-003, FR-004, FR-009, FR-014 | 学业入口、PDF 候选明细、常用模板、进度中心与 AI 卡片官方链接展示均映射到既有 FR 的增量验收 |
...
- `S12` 继续沿用“增量验收条目吸收边界”的做法：默认学生与培养方案导入只消费仓库登记数据源，不从学号或预计毕业年份推断专业、年级、班级，`2024-default` 仅作为初始演示版本。
